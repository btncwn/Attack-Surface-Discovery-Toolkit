...
with st.form("scan_form"):
    ...
    submitted = st.form_submit_button("🔍 Scan Target")
...
if submitted:
    ...
    with st.spinner("Running attack surface scan..."):
...
if 'previous_scan' in locals() and previous_scan:
                    ...
                    if isinstance(item, list):
                        for sub_item in item:
                            if (
                                ...
                                and len(sub_item) >= 2
                                and sub_item[0] in ["organizationName", "commonName"]
                            ):
    ...
    with col1:
        st.metric("Attack Surface Score", f"{executive_summary['score']}/100")
    ...
    st.write("Prioritize these findings based on severity and expected score impact.")
    ...
    with st.expander("📊 Summary Statistics"):
        ...
        with col2:
            st.write(f"**Hidden Assets:** {executive_summary['asset_count']}")
            st.write(f"**CVEs Detected:** {executive_summary['cve_count']}")
            st.write(
                "**Findings:** "
    ...
    with st.expander("🧬 CVE Correlation Engine"):
        if cve_results:
            ...
            st.write(
                ...
                f"{len(cve_results)} detected technologies."
            )
            ...
            for technology, cves in cve_results.items():
                ...
                for cve in cves:
                    emoji = "🔴" if cve.get("severity") == "CRITICAL" else "🟡"
                    exploit_emoji = " 💀" if cve.get("has_exploit") else ""

                    st.write(
                        f"{emoji} **{cve.get('cve_id')}** "
                        f"- CVSS: {cve.get('cvss_score')} "
                        f"({cve.get('severity')}){exploit_emoji}"
                    )
                    st.caption(cve.get("description", ""))
    ...
    with st.expander("🌍 Shodan Intelligence"):
        if shodan_info:
            ...
            st.info(
                "No Shodan API key provided, no A record found, "
                "or API plan does not support host lookup."
            )
    ...
    with st.expander("🕵️ Hidden Asset Discovery"):
        ...
        if hidden_assets:
            st.warning(
                "⚠️ These assets were found in Certificate Transparency logs "
                "but NOT in standard enumeration"
            )
